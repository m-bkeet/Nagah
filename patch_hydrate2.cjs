...
const oldHydrate = `      console.log(\`[Hydration] Successfully loaded \${data.length} documents from Supabase public.collections across \${Object.keys(grouped).length} collections.\`);
      return data.length;
    }
  } catch (err: any) {`;

const newHydrate = `      console.log(\`[Hydration] Successfully loaded \${data.length} documents from Supabase public.collections across \${Object.keys(grouped).length} collections.\`);
