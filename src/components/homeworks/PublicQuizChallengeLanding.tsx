...
export const PublicQuizChallengeLanding: React.FC<PublicQuizChallengeLandingProps> = ({
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-6 text-center">
        <div className="w-16 h-16 rounded-3xl bg-amber-500/20 border border-amber-500/40 flex items-center justify-center text-amber-400 mb-4 animate-bounce">
          <Sparkles className="w-8 h-8" />
        </div>
        <h2 className="text-xl font-black text-slate-100 mb-2">جاري تجهيز تحدي كاهوت التفاعلي...</h2>
  ...
  if (error || !assignment) {
    return (
      <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-6 text-center">
        ...
        <h2 className="text-xl font-black text-slate-100 mb-2">تعذر العثور على المسابقة</h2>
        ...
        <div className="flex flex-wrap items-center justify-center gap-3">
          ...
          {onBack && (
            <button
              onClick={onBack}
              className="px-5 py-2.5 rounded-2xl bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold text-xs transition-all border border-slate-700"
            >
              العودة للرئيسية
            </button>
          )}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-white flex flex-col items-center justify-center p-4 sm:p-6 relative overflow-hidden" dir="rtl">
      ...
      {/* Main Challenge Card */}
      <div className="w-full max-w-xl bg-slate-900/90 border border-slate-800/90 rounded-3xl p-6 sm:p-8 shadow-2xl backdrop-blur-xl relative z-10 space-y-6">
        
        {/* Header Badges */}
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800/80 pb-4">
          <div className="flex items-center gap-2">
            <span className="w-10 h-10 rounded-2xl bg-amber-500/20 border border-amber-500/40 flex items-center justify-center text-amber-400 shrink-0 shadow-inner">
          ...
          {assignment.evaluationSource && (
            <span className="text-[11px] font-bold text-emerald-300 bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-1 rounded-xl flex items-center gap-1.5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
          <div className="bg-slate-950/60 border border-slate-800/80 rounded-2xl p-3">
            <span className="text-[10px] text-slate-400 font-bold block mb-1">المادة / المقرر</span>
          ...
          <div className="bg-slate-950/60 border border-slate-800/80 rounded-2xl p-3">
            <span className="text-[10px] text-slate-400 font-bold block mb-1">عدد الأسئلة</span>
          ...
          <div className="bg-slate-950/60 border border-slate-800/80 rounded-2xl p-3">
            <span className="text-[10px] text-slate-400 font-bold block mb-1">المؤقت لكل سؤال</span>
          ...
          <div className="bg-slate-950/60 border border-slate-800/80 rounded-2xl p-3">
            <span className="text-[10px] text-slate-400 font-bold block mb-1">الدرجة المستحقة</span>
        ...
        {gameResult && (
          <div className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 space-y-2">
            ...
            <p className="text-xs text-slate-300">
              حصلت على: <span className="font-bold text-amber-400">{gameResult.score || 0}</span> من أصل <span className="font-bold text-white">{assignment.totalMarks || 100}</span> ({gameResult.percentage || 0}%)
            </p>
          </div>
        )}
        ...
        <div className="space-y-3 bg-slate-950/50 border border-slate-800/80 rounded-2xl p-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-slate-900 border border-slate-700/80 focus:border-amber-500 rounded-xl pr-9 pl-3 py-2.5 text-xs sm:text-sm text-white focus:outline-none transition-colors"
            />
          </div>
        ...
        <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-slate-800/60 text-xs">
          {onGoToPortal && (
            <button
              type="button"
              onClick={onGoToPortal}
