...
export const GoogleFormsImportModal: React.FC<GoogleFormsImportModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/75 backdrop-blur-sm" dir="rtl">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col text-slate-900 dark:text-slate-100 animate-in fade-in zoom-in-95">
        <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-bold text-slate-900 dark:text-white text-lg">استيراد بيانات من Google Forms</h2>
              <p className="text-xs text-slate-500 dark:text-slate-400">اختر نموذج تسجيل لاستيراد استجابات الطلاب</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-slate-700 dark:hover:text-white transition-all cursor-pointer">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-4 flex-1 overflow-y-auto max-h-[60vh] custom-scrollbar">
          {error && (
            <div className="p-4 mb-4 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-500/30 rounded-xl flex items-center gap-3 text-red-700 dark:text-red-400 text-sm">
          ...
          {isFetching ? (
            ...
            <div className="space-y-4">
              <div>
                <label className="text-sm font-bold text-slate-800 dark:text-white mb-2 block">اختر النموذج المطلوب استيراده</label>
                <select 
                  value={selectedFormId}
                  onChange={(e) => setSelectedFormId(e.target.value)}
                  className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white text-sm rounded-xl px-4 py-3 focus:outline-none focus:border-emerald-500 transition-colors"
                >
              ...
              <div className="p-4 rounded-xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 text-emerald-800 dark:text-emerald-300 text-xs leading-relaxed">
                <strong className="block mb-1 text-emerald-900 dark:text-emerald-400">ملاحظة:</strong>
        ...
        <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/50 flex justify-end gap-3">
          <button
            onClick={onClose}
            className="px-5 py-2 rounded-xl text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all font-bold text-sm cursor-pointer"
          >
            إلغاء
          </button>
          <button
            ...
            className="px-5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-bold disabled:opacity-50 transition-all flex items-center gap-2 cursor-pointer shadow"
          >
